...
function test_scripts() {
	...
	if(is_page_template('ajax-test.php')) {
		wp_enqueue_script( 'test-ajax', get_template_directory_uri() . '/js/ajax-test.js', array('jquery'), time(), true );
		$ajaxurls = admin_url('admin-ajax.php');
		wp_localize_script('test-ajax', 'urls',['ajaxurl' => $ajaxurls]);
	}
}
add_action( 'wp_enqueue_scripts', 'test_scripts' );

function test_ajax_test() {
	if(check_ajax_referer('ajax_test', 's')) {
		$info = $_POST['info'];
		echo strtoupper($info);
		die();
	}
}
add_action('wp_ajax_ajax_test', 'test_ajax_test');
function test_ajax_test_nopriv() {
	if(check_ajax_referer('ajax_test', 's')) {
		$info = $_POST['info'];
		echo strtoupper("Golobal".$info);
		die();
	}
}
add_action('wp_ajax_nopriv_ajax_test', 'test_ajax_test_nopriv');
